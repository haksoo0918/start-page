...
describe('날씨 서비스 단위 테스트 (TDD Seam)', () => {
  ...
  it('기본 지역은 고양시 일산동구여야 한다', () => {
    expect(DEFAULT_REGION.id).toBe('ilsan-east');
    expect(DEFAULT_REGION.name).toBe('고양시 일산동구');
    expect(DEFAULT_REGION.lat).toBeCloseTo(37.658, 2);
    expect(DEFAULT_REGION.lng).toBeCloseTo(126.772, 2);
  });
});
